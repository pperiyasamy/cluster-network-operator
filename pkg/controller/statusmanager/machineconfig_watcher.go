...
func (s *StatusManager) AddMachineConfigWatcher(mgr manager.Manager) error {
	...
	return c.Watch(source.Kind(operatorCache, &mcfgv1.MachineConfig{}),
...
func (p *MachineConfigWatcher) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil && apierrors.IsNotFound(err) {
		klog.Infof("processing deleted machine config: %s", request.NamespacedName.Name)
		p.status.processDeletedMachineConfig(request.NamespacedName.Name)
		return reconcile.Result{}, nil
	}
	klog.Infof("processing created machine config: %s", request.NamespacedName.Name)
	if err != nil {
		klog.Infof("failed to retrieve machine config: %v", err)
		return reconcile.Result{}, nil
	}
...
func (s *StatusManager) AddMachineConfigPoolWatcher(mgr manager.Manager) error {
	...
	return c.Watch(source.Kind(operatorCache, &mcfgv1.MachineConfigPool{}),
...
func (p *MachineConfigPoolWatcher) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		klog.Infof("failed to retrieve machine config pools: %v", err)
		return reconcile.Result{}, nil
	}
	klog.Infof("processing machine config pools %d: %v", len(mcPools.Items), mcPools)
