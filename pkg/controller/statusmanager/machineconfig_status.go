...
func (status *StatusManager) SetFromMachineConfigPool(mcPools []mcfgv1.MachineConfigPool) {
	...
	klog.Infof("processing master and worker MCs %v: %v", masterMCs, workerMCs)
	...
	klog.Infof("processing deleted master and worker MCs 1 %v: %v", masterMCs, workerMCs)
	...
	klog.Infof("processing deleted master and worker MCs 2 %v: %v", masterMCs, workerMCs)
...
func (status *StatusManager) updateNetworkStatus(mcPools []mcfgv1.MachineConfigPool, mcRole string, machineConfigs sets.Set[string],
	...
	if err != nil {
		klog.Infof("failed to get machine config pools for the label %s: %v", mcLabel, err)
	}
	klog.Infof("processing pools %v for label %v", pools, mcLabel)
...
func (status *StatusManager) processCreatedMachineConfig(mc mcfgv1.MachineConfig) {
	...
	if role, exists := mc.Labels[platform.MachineConfigLabelRoleKey]; exists {
		...
		klog.Infof("machine config %s doesn't have %s label, skipping it", mc.Name, platform.MachineConfigLabelRoleKey)
		return
	}
	status.Lock()
	defer status.Unlock()
